...
STRINGS = {
    ...
    'wv_curve_ushape': ('{gain_txt}。曲线呈 **U 型**：最低点在约 {p_min} cmH2O，之后随压力回升'
                        '（最高档比最低点高 {rise}）。高段的回升是设备对气流受限的**反应性升压**'
                        '（回声），不能读作「压力有害」；而它一路升到区间顶端，正是「还想要更多压力'
    ...
    'pev_split': ('按实际送达压力切：低段（<{p90} cmH2O，P90）每 100 拍 {lo_rate} 个事件，'
                  '高段（≥{p90}）{hi_rate} 个（低段 {lo_br} 拍 / 高段 {hi_br} 拍）。⚠ 该比率**不是**'
                  '「压力有没有效」的证据：设备在事件后会主动升压（见上一条反馈检验），高段停留本身'
                  '由事件触发，「高段事件多」是升压反馈的回声。它只说明设备确实在高段工作。'),
    ...
    'report_s4_ratio': ('- 高压段/其余时段的事件率比 {rr}（按压力采样点暴露校正）。**该比值不可读作'
                        '「加压无效」**：APAP 在事件后主动升压，高段停留本身就是事件触发的（第 4b 节'
                        '有反馈检验），共现是回声不是因果。'),
    'report_s4_spearman': '- 跨夜 Spearman ρ(压力95, OAI)={rho}（n={n}）；APAP 具有事件后升压的反向因果，不能做因果解读。',
    ...
    'detail_oa_press_none': '阻塞事件发生时压力 —',
