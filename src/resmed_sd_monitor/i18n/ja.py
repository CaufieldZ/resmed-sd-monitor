...
STRINGS = {
    ...
    'wv_cluster': ('イベント クラスタリング：{nights} 晩で {clusters} クラスタ（≥{min_ev} 件・隣接間隔'
                   ...
                   '体位性/REM 関連 OSA の典型的な手がかりだが、装置は体位も睡眠段階も記録しない——'
                   'これだけで帰属してはいけない。'),
    ...
    'wv_curve_ushape': ('{gain_txt}。曲線は **U 字型**：約 {p_min} cmH2O が最低で、以後圧力とともに再上昇'
                        '（最上位ビンは最低点より {rise} 上）。高圧側の回復は流量制限への**反応的**昇圧'
                        '（こだま）であり「圧力が有害」ではない。そして区間の上限まで昇り続けているのは'
                        '「さらに圧力を欲しているが上限に阻まれている」形——「圧力—平坦化曲線がこの区間で'
                        'まだ伸びきっていない」ことと同じ事実。'),
    ...
    'pev_split': ('実際の送達圧で分割：低圧帯（<{p90} cmH2O、P90）は 100 呼吸あたり {lo_rate} 件、'
                  '高圧帯（≥{p90}）は {hi_rate} 件（低圧 {lo_br} 呼吸 / 高圧 {hi_br} 呼吸）。⚠ この比は'
                  '「圧力が効くか」の証拠では**ない**：装置はイベント後に昇圧し（上のフィードバック検証）、'
                  '高圧帯への滞留自体がイベントで引き起こされる。「高圧帯でイベントが多い」はその'
                  'フィードバックのこだま。装置が確かに高圧帯で動作していることだけを示す。'),
    ...
    'wave_feedback_confirmed': '装置はイベントに反応して昇圧している——イベント率比はこだまであり、圧力無効の証拠にはならない。',
    ...
    'wave_merge_climbing': ('· 統合：曲線の底は中圧域にあり、右端がまだ上っている（セグメント間差 {gain}）'
                            '——さらに圧力を稼げる余地があり、低圧側はすでに最適点付近。**下限を上げても'
                            '無駄。上限を上げるべき**。'),
    ...
    'wave_merge_responsive': ('· 統合：背景制限は圧力に対して単調に改善（セグメント間差 {gain}）。低圧側に'
                              'まだ余地がある——下限圧を上げる価値はある。1〜2 週間後に再評価。残存イベント'
    ...
    'report_s4_ratio': ('- 高圧帯/その他のイベント率比 {rr}（圧力サンプルの曝露で補正）。**この比を'
                        '「昇圧が無効」と読んではいけない**：APAP はイベント後に昇圧し、高圧帯への滞留は'
                        'それ自体がイベント由来（セクション 4b のフィードバック検証）。共起はこだまであり'
                        '因果ではない。'),
    'report_s4_spearman': '- 晩をまたぐ Spearman ρ(圧力 P95, OAI)={rho}（n={n}）。APAP にはイベント後昇圧という逆因果があり、因果として読めない。',
    ...
    'detail_oa_press_none': '閉塞イベント発生時の圧力 —',
