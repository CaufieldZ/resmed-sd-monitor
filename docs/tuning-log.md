...
prescription changes, append one entry. That is what lets the report split a pile
of nights into a segmented before/after comparison.
...
- `label`: segment name shown in tables and chart colour bands. Defaults to the date.
...
- `note`, `device`, `mask`: free text and report header, optional.
...
One line per change, and keep `label` short: it is printed on the charts.
