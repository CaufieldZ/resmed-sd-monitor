...
- `Central Apnea` → `ca`; `Obstructive Apnea` → `oa`; anything containing
  `hypopnea` → `hyp`.
- A bare `Apnea` (no qualifier) stays **unclassified** (`ua`) — never silently
...
- **One night can have multiple files of each type** (therapy resumed after a
  mask-off). Therapy intervals are computed as the *union* of BRP segments —
  gaps are not counted as exposure, overlaps are not double-counted. Events
...
cannot support T90 and kept inviting exactly the misreading the quality gates
exist to prevent.
